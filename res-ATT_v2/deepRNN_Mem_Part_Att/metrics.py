...
def generate_sample_gpu_single_process(
    ...
    if samples_valid:
        ...
        with open(os.path.join(save_dir, str(epoch)+'_valid_samples.txt'), 'w') as f:
            for key, value in samples_valid.items():
                print >>f, key+': '+value[0]['caption']+'\n'
    if samples_test:
        samples_test = build_sample_pairs(samples_test, engine.test_ids)
        with open(os.path.join(save_dir, str(epoch)+'_test_samples.txt'), 'w') as f:
            for key, value in samples_test.items():
                print >>f, key+': '+value[0]['caption']+'\n'
