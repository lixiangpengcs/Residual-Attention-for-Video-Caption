...
class Predict(object):
    ...
    def build_sampler(self, layers, tparams, options, use_noise, trng):
        ...
        mu_init_state = [layers.get_layer('ff')[1](tparams, ctx_mean,
                                                   prefix='ff_state', activ='tanh')]
        mu_init_memory = [layers.get_layer('ff')[1](tparams, ctx_mean,
        ...
        tu_lstm = layers.get_layer('lstm')[1](tparams, emb, one_step=True,
                                              ...
                                              prefix='tu_lstm')
        attend = layers.get_layer('attend')[1](tparams, tu_lstm[0], ctx_,
                                               one_step=True)
        mu_lstm = layers.get_layer('lstm_concat')[1](options, tparams, tu_lstm[0],
                                                     mask=None, ctxs=attend[1],
                                                     one_step=True,
    ...
    def sample_execute(self, engine, options, tparams, f_init, f_next, x, ctx, mask_ctx, trng,vid_names=None):
        stochastic = False
        for jj in xrange(np.minimum(10, x.shape[1])):
            if vid_names != None:
                print 'vid_names: ',vid_names[jj]
